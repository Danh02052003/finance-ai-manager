...
const ExternalDebtTable = ({ items, onEdit, onDelete, onSettle }) => {
  ...
  <section className="rounded-3xl border border-white/[0.08] bg-black/20 backdrop-blur-md p-6 shadow-2xl">
    <div className="flex items-center justify-between mb-6">
      <h3 className="text-lg font-bold text-white tracking-tight">{t('debts.listTitle', 'Danh sách nợ')}</h3>
      <div className="text-sm font-medium text-slate-400">
        {t('debts.totalCount', 'Tổng số: ')}<span className="text-white">{items.length}</span>
      </div>
    </div>
    ...
    <div className="space-y-4 md:hidden">
      {items.length > 0 ? (
            ...
            <div className="relative">
              <div className="flex items-center justify-between mb-3">
                <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest ${statusStyles[item.status] || 'bg-white/[0.06] text-slate-400 border border-white/10'}`}>
                  {item.status === 'open' ? t('debts.statusOpen', 'Đang mở') : t('debts.statusSettled', 'Đã tất toán')}
                </span>
              ...
              <div className="flex items-center gap-2 mb-4 text-sm bg-white/[0.02] p-3 rounded-xl border border-white/[0.02] justify-center">
                <span className="text-slate-400">
                  {item.debt_type === 'borrowed' ? t('debts.typeBorrowed', 'Mượn của:') : t('debts.typeLent', 'Cho mượn:')}
                </span>
                <span className="font-semibold text-indigo-300 ml-1">{item.creditor_name}</span>
              </div>

              <div className="flex items-end justify-between mb-4">
                <div>
                  <p className="text-[11px] font-medium uppercase tracking-wider text-slate-500 mb-1">{t('debts.amount', 'Số tiền')}</p>
        ...
        <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-white/[0.1] bg-white/[0.02] py-16 px-6 text-center">
          ...
          <p className="text-base font-medium text-white mb-1">{t('debts.noDebts', 'Chưa có khoản nợ nào')}</p>
        </div>
      )}
    </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          <thead className="bg-white/[0.04] border-b border-white/[0.08]">
            <tr>
              <th className="px-5 py-4 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[25%]">{t('debts.info', 'Thông tin')}</th>
              <th className="px-5 py-4 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[20%]">{t('debts.type', 'Loại nợ')}</th>
              <th className="px-5 py-4 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[30%]">{t('debts.person', 'Người mượn/cho mượn')}</th>
              <th className="px-5 py-4 text-right text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[25%]">{t('debts.amount', 'Số tiền')}</th>
            </tr>
          </thead>
                ...
                <tr key={item._id || `ext-${index}`} className="group transition hover:bg-white/[0.02]">
                  ...
                  <td className="px-5 py-4">
                    <div className="text-sm">
                      <span className={`px-2 py-1 rounded text-xs ${item.debt_type === 'borrowed' ? 'bg-rose-500/20 text-rose-300' : 'bg-emerald-500/20 text-emerald-300'}`}>
                        {item.debt_type === 'borrowed' ? t('debts.borrowed_label', 'Mượn người khác') : t('debts.lent_label', 'Cho mượn')}
                      </span>
                    </div>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-right relative">
                    <div className={`text-base font-bold tabular-nums group-hover:opacity-0 transition-opacity duration-200 ${item.debt_type === 'borrowed' ? 'text-rose-400' : 'text-emerald-400'}`}>
                      {item.debt_type === 'borrowed' ? '-' : '+'}{typeof item.amount === 'number' ? formatCurrency(item.amount) : '-'}
                    </div>
...
export default ExternalDebtTable;
