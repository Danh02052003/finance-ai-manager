...
const DebtTable = ({ items, jars = [], onEdit, onDelete, onSettle }) => {
  ...
  const getJarName = (key) => {
    ...
    return jar ? (i18n.language === 'en' ? jar.jar_key.replace(/_/g, ' ').toUpperCase() : jar.display_name_vi) : key;
  };
    ...
    <div className="space-y-4 md:hidden">
      {items.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-white/[0.1] bg-white/[0.02] py-16 px-6 text-center">
          ...
          <p className="text-sm text-slate-500">{t('debts.noDebtsDesc', 'Các khoản tạm ứng giữa các hũ sẽ xuất hiện ở đây.')}</p>
        </div>
      )}
    </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          <thead className="bg-white/[0.04] border-b border-white/[0.08]">
            <tr>
              <th className="px-5 py-4 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[30%]">{t('debts.info', 'Thông tin')}</th>
              <th className="px-5 py-4 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[40%]">{t('debts.transaction', 'Giao dịch')}</th>
              <th className="px-5 py-4 text-right text-[11px] font-bold uppercase tracking-widest text-slate-400 w-[30%]">{t('debts.amount', 'Số tiền')}</th>
            </tr>
          </thead>
